...
class SpectralDomainData(TimeDomainData):
    ...
    def np_spec(self, data):
        data = librosa.stft(data, n_fft=self.frame_size, hop_length=self.frame_size // 2,
    ...
    def tf_spec(self, data):
        data = tf.signal.stft(data, self.frame_size, self.frame_size // 2, self.frame_size)
        if self.dc is False:
            data = cut_dc(data)
...
class SpectralMagnitudeData(SpectralDomainData):
    ...
    def dataset(self, x_y_snr, form='log'):  # @overrides
        ...
        if form == 'log':
            x_data = np.log10(x_data)  # ******************************************log 0?

        if self.y_form != 'time':
            y_data = self.np_spec(y_data)
            y_data, y_phase = librosa.magphase(y_data)
            if form == 'log':
                y_data = np.log10(y_data)  # ******************************************log 0?
